								...
								<div class="col-xs-6">
									<div class="bill-data">
										...
										<address>
											...
											echo translate('company').' : '. $billdata['supplier_company_name'] . '<br>';
							...
							<table class="table invoice-items table-hover mb-none">
								<thead>
									<tr class="text-dark">
										...
										<th id="cell-item" class="text-weight-semibold"><?php echo translate("chemical"); ?></th>
								...
								<tbody>
									<?php
										$count = 1;
										$chemicallist = $this->inventory_model->get_list('purchase_bill_details', array('purchase_bill_id' => $billdata['id']));
										foreach ($chemicallist as $chemical) {
											$sub_total = $chemical['sub_total'];
											$discount = $chemical['discount'];
									?>
									<tr>
										<td><?php echo $count++; ?></td>
										<td class="text-weight-semibold text-dark"><?php echo get_type_name_by_id('chemical', $chemical['chemical_id']); ?></td>
										<td><?php echo html_escape($currency_symbol . $chemical['unit_price']); ?></td>
										<td><?php echo html_escape($chemical['quantity']); ?></td>
										<td><?php echo html_escape($currency_symbol . $discount); ?></td>
										<td class="text-center"><?php echo html_escape($currency_symbol . ($sub_total - $discount)); ?></td>
									</tr>
									<?php } ?>
								</tbody>
							</table>
						...
						<div class="invoice-summary text-right mt-lg">
							<div class="row">
								<div class="col-md-5 pull-right">
									<ul class="amounts">
										<li><?php echo translate("sub") . " " . translate('total'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_amount); ?></li>
										<li><?php echo translate('discount'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_discount); ?></li>
<?php if ($status == 3){ $g = ($total_amount - $total_discount); $grandtotal = number_format($g, 2, '.', '') ?>
										<li>
											<strong><?php echo translate('grand') . " " . translate('total'); ?> (<?php echo html_escape($currency_symbol); ?>) : </strong> 
											<?php
											$f = new NumberFormatter("en", NumberFormatter::SPELLOUT);
											echo html_escape($grandtotal) . " </br>( ". ucwords($f->format($grandtotal)) . " $currency )";
											?>
										</li>
<?php }else{ ?>
										<li><?php echo translate('paid_amount'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_paid); ?></li>
										<li>
											<strong><?php echo translate('due'); ?> (<?php echo html_escape($currency_symbol); ?>) :</strong> 
											<?php
											$f = new NumberFormatter("en", NumberFormatter::SPELLOUT);
											echo html_escape($due_amount) . ' </br>( ' . ucwords($f->format($due_amount)) . ' )';
											?>
										</li>
<?php } ?>
									</ul>
						...
						<div class="table-responsive">
							<table class="table invoice-items table-hover mb-none">
								...
								<tbody>
									<?php
										$count = 1;
										$paymentlist = $this->inventory_model->get_list('purchase_payment_history', array('purchase_bill_id' => $billdata['id']));
										foreach ($paymentlist as $payment) {
									?>
									<tr>
										<td><?php echo $count++; ?></td>
										<td class="text-weight-semibold text-dark"><?php echo get_type_name_by_id('staff', html_escape($payment['payment_by'])) ; ?></td>
										<td><?php echo get_type_name_by_id('payment_type', $payment['pay_via']) ; ?></td>
						...
						<div class="invoice-summary text-right mt-lg">
							<div class="row">
								<div class="col-lg-5 pull-right">
									<ul class="amounts">
										<li><?php echo translate('total'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_amount); ?></li>
										<li><?php echo translate('discount'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_discount); ?></li>
										<li><?php echo translate('paid_amount'); ?> (<?php echo html_escape($currency_symbol); ?>) : <?php echo html_escape($total_paid); ?></li>
										<li>
											<strong><?php echo translate('due'); ?> (<?php echo html_escape($currency_symbol); ?>) :</strong> 
...
<?php if($billdata['payment_status'] != 3 && get_permission('purchase_payment', 'is_add')): ?>
			...
			<div id="add_payment" class="tab-pane <?php echo ($active_tab == 3 ? 'active' : ''); ?>">
				<?php echo form_open(base_url('inventory/add_payment'), array('class' => 'form-horizontal form-bordered validate', 'method' => 'post', 'enctype' => 'multipart/form-data'));?>
					<input type="hidden" name="purchase_bill_id" value="<?php echo html_escape($billdata['id']); ?>">
					<div class="form-group <?php if (form_error('paid_on')) echo 'has-error'; ?>">
					...
					<div class="form-group <?php if (form_error('payment_amount')) echo 'has-error'; ?>">
